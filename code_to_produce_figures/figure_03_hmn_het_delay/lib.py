...
def plot_correlation_distribution(
    ...
    plot_hist(corOut, axs, bins=nbins, color='red',
              label="inter", **kwargs)
    plot_hist(corIn, axs, bins=nbins, color='royalblue',
              label="intera", **kwargs)
    if label:
        axs.text(0.05, 0.4, str(r"$\nu_0 = %.0f$Hz" % label),
                 fontsize=13, transform=axs.transAxes)
    axs.tick_params(labelsize=14)
    axs.text(-0.25, 0.9, label1,
...
def plot_hist(arr, ax, bins='auto', color='red',
              label=None,
              xticks=None,
              yticks=None,
              xlim=None,
    ...
    hist, bins = np.histogram(arr, bins=bins, normed=False, range=bin_range)
    width = (bins[1] - bins[0])
    ...
    if ylabel:
        ax.set_ylabel("Counts", fontsize=14, labelpad=0)
